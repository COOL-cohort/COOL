...
//        CohortQuery query = mapper.readValue(new File("query.json"), CohortQuery.class);
        ...
        List<ResultTuple> resultTuples = executeQuery(coolModel.getCube(query.getDataSource()), query, map);
        QueryResult result = QueryResult.ok(resultTuples);
        System.out.println(result.toString());
        coolModel.close();
    }

    public static List<ResultTuple> executeQuery(CubeRS cube, CohortQuery query, Map<String, DataOutputStream> map) throws IOException {
        ...
        for (CubletRS cublet : cublets) {
            ...
            for (Map.Entry<CohortKey, Long> entry : results.entrySet()) {
                ...
                String cohort = converter == null ? cohortMetaField.getString(key.getCohort()) : converter.getString(cId);
                int age = key.getAge();
