...
import com.nus.cool.core.cohort.filter.*;
import com.nus.cool.core.cohort.BirthSequence.Anchor;
import com.nus.cool.core.cohort.BirthSequence.BirthEvent;
import com.nus.cool.core.cohort.BirthSequence.CohortField;
import com.nus.cool.core.cohort.BirthSequence.TimeWindow;
import com.nus.cool.core.cohort.ExtendedFieldSet.FieldValue;
 ...
 *
 */
public class ExtendedCohortSelection implements CohortOperator {

	// private static Log LOG =
	// LogFactory.getLog(ExtendedCohortSelectionV2.class);

	private FieldFilterFactory filterFactory = new FieldFilterFactory();

	private List<Map<String, FieldFilter>> birthFilters = new ArrayList<>();

	private List<Map<String, FieldFilter>> birthAggFilters = new ArrayList<>();
	...
	private Map<String, FieldFilter> ageByFilters = new HashMap<>();
	...
	private ArrayList<LinkedList<Integer>> eventOffset = new ArrayList<>();

	private ExtendedCohort cohort = new ExtendedCohort();
	...
	@Override
	public void init(CubeSchema cubeSchema, TableSchema tableSchema, InputVector cohortUsers,
	...
	public void init(TableSchema tableSchema, ExtendedCohortQuery q) {
		...
		for (BirthSequence.BirthEvent e : seq.getBirthEvents()) {
			...
			for (ExtendedFieldSet fs : e.getEventSelection()) {
				...
				checkArgument(schema.getDataType() != DataType.Aggregate);
				filters.put(fn,
		...
		for (int i = 0; i < 2; i++) {
			...
			for (ExtendedFieldSet fs : selectors) {
				...
				filterMap.put(field, filterFactory.create(tableSchema.getField(field), fs,
	...
	public void process(MetaChunkRS metaChunk) {
		...
		this.maxDate = TimeUtils.getDate(
				metaChunk.getMetaField(tableSchema.getActionTimeField(), FieldType.AppKey).getMaxValue());
	}
		...
		for (int i = 0; r && i < birthAggFilters.size(); i++) {
			...
			for (Map.Entry<String, FieldFilter> entry : filters.entrySet()) {
				...
				if (entry.getValue() instanceof SetFieldFilter) {
					...
					FieldValue value = filter.getFieldSet().getFieldValue();
	...
	private int getUserBirthTime(int start, int end) {
		...
		for (Integer e : sortedEvents) {
			...
			// check time window
			TimeWindow window = event.getTimeWindow();
			if (window == null) {
				// no time window
				...
				for (Anchor anc : window.getAnchors()) {
	...
	public ExtendedCohort selectUser(int start, int end) {
		...
		if (boff >= 0) {
			...
			List<BirthEvent> events = q.getBirthSequence().getBirthEvents();
				...
				for (CohortField cf : be.getCohortFields()) {
					...
					if (schema.getDataType() == DataType.String) {
						...
						if (cf.isLogScale()) {
							...
							v = (value / cf.getScale());
							level = (v > (int) v) ? (int) v +1 : (int) v;
							level = Math.max(level, cf.getMinLevel());
	...
	private void filterAgeActivity(int ageOff, int ageEnd, BitSet bs, InputVector fieldIn, 
		...
		FieldValue value = ageFilter.getFieldSet().getFieldValue();
		if (value.getType() != FieldValueType.AbsoluteValue) {
			ageFilter.updateValues(
					getBirthAttribute(value.getBaseEvent(), tableSchema.getFieldID(value.getBaseField())));
		}
