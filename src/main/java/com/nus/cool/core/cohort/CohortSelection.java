...
public class CohortSelection implements Operator {
    ...
    public void init(TableSchema schema, CohortQuery query) {
        ...
        this.appFilter = FieldFilterFactory.create(this.schema.getField(this.schema.getAppKeyField()), Arrays.asList(app));

        List<FieldSet> birthSelectors = query.getBirthSelection();
        for (FieldSet fs : birthSelectors) {
            String fieldName = fs.getField();
            this.birthFilters.put(fieldName, FieldFilterFactory.create(this.schema.getField(fieldName), fs.getValues()));
        }

        List<FieldSet> ageSelectors = query.getAgeSelection();
        for (FieldSet fs : ageSelectors) {
            String fieldName = fs.getField();
            this.ageFilters.put(fieldName, FieldFilterFactory.create(this.schema.getField(fieldName), fs.getValues()));
        }
    }
    ...
    public void process(MetaChunkRS metaChunk) {
        ...
        boolean bAccept = this.appFilter.accept(metaChunk.getMetaField(this.schema.getAppKeyField(), FieldType.AppKey));
        if (!bAccept) {
