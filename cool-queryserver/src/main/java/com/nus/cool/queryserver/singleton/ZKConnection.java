...
public class ZKConnection {
    ...
    public void connect() throws IOException, InterruptedException {
        ...
        zk = new ZooKeeper(ZK_HOST, ZK_PORT, watchedEvent -> {
            if (watchedEvent.getState() == Watcher.Event.KeeperState.SyncConnected) latch.countDown();
        });
        latch.await();
    }
     ...
     */
    public void addWorker(String serverHost) throws IOException, KeeperException, InterruptedException {
        ...
        if (workerExist == null){
            zk.create("/workers", "all workers".getBytes(StandardCharsets.UTF_8), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
        }
