...
public class CoolCohortEngine {
  ...
  public List<String> listCohortUsers(CubeRS cube, List<Integer> inCohort) {
    ...
    MetaFieldRS metaField = metaChunk.getMetaField(tableSchema.getUserKeyField(),
        FieldType.UserKey);
   ...
   */
  public List<Integer> selectCohortUsers(CubeRS cube, InputVector users, ExtendedCohortQuery query)
      throws IOException {
    if (cube == null) {
   ...
   */
  public void createCohort(ExtendedCohortQuery query, List<Integer> users, File cohortRoot)
      throws IOException {
    ...
    Compressor compressor = new ZIntBitCompressor(Histogram.builder().max(maxValue)
        .numOfValues(userArray.length).uniqueValues(userArray.length).build());
    byte[] compressed = new byte[compressor.maxCompressedLength()];
    int nbytes = compressor.compress(userArray, 0, userArray.length, compressed, 0,
        compressed.length);
  ...
  /**
   * Process cohort results.
   */
  public List<ExtendedResultTuple> processCohortResult(ExtendedCohortQuery query,
      TableSchema tableSchema, MetaChunkRS metaChunk, ExtendedCohortSelection sigma,
      ...
      for (int idx = 0; idx < events.size(); ++idx) {
        ...
        for (BirthSequence.CohortField cf : be.getCohortFields()) {
          ...
          if (schema.getDataType() == DataType.String) {
            key.addDimensionName(
                metaChunk.getMetaField(fieldName).getString(key.getDimensions().get(cnt)));
          } else {
            MetaFieldRS field = metaChunk.getMetaField(fieldName);
            StringBuilder builder = new StringBuilder();
            builder.append("(");
            int level = key.getDimensions().get(cnt);
            if (cf.getMinLevel() == level) {
              builder.append(Integer.toString(field.getMinValue()) + ", ");
            } else {
              if (cf.isLogScale()) {
            ...
            if (cf.getMinLevel() + cf.getNumLevel() == level) {
              builder.append(Integer.toString(field.getMaxValue()) + ")");
            } else {
              if (cf.isLogScale()) {
   ...
   * @return result
   */
  public List<ExtendedResultTuple> performCohortQuery(CubeRS cube, InputVector users,
  ...
  /**
   * Export all records of a cohort to a separate cube.
   */
  public boolean exportCohort(CubeRS cube, InputVector users, DataWriter writer)
      throws IOException {
