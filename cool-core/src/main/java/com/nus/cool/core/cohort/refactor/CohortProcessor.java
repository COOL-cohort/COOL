...
public class CohortProcessor {
  ...
  /**
   * Create cohort processor from its serialized representation.
   */
  ...
  private HashMap<String, int[]> loadInvariantGidMaps(UserMetaFieldRS userMetaField) {
    ...
    for (String invariantFieldName : invariantName) {
      invariantRangeGidMap.put(invariantFieldName,
          userMetaField.getInvariantGidMap(invariantFieldName));
    }
    return invariantRangeGidMap;
  }

  /**
   * In this section, we load the tuple which is an inner property. We left the process logic in
   * processTuple function.
   *
  ...
  private void processDataChunk(ChunkRS chunk, MetaChunkRS metaChunk,
      HashMap<String, String[]> hashMapperBySchema, HashMap<String, int[]> invariantGidMap) {
  ...
  private void processTuple() {
    ...
    LocalDateTime actionTime = DateUtils
        .daysSinceEpoch((int) tuple.getValueBySchema(this.actionTimeSchema));
  ...
  private Boolean checkMetaChunk(MetaChunkRS metaChunk) {
    ...
    for (EventSelection es : this.birthSelector.getBirthEvents()) {
      for (Filter ft : es.getFilterList()) {
        ...
        if (this.checkMetaField(metaField, ft)) {
          return false;
        }
      }
    }
  ...
  /**
   * Check if filter condition is likely to be met based on metafield info.
   */
