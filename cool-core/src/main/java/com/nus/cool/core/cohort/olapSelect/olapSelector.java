...
public class olapSelector {
  ...
  public void init(TableSchema tableSchema, OlapQueryLayout query) throws ParseException {
    ...
    olapSelectionLayout selection = query.getSelection();

    this.filter = initFilter(selection);
    ...
    if (query.getTimeRange() != null) {
      ...
      granularityType granularity = query.getGranularity();
      ...
      splitTimeRange(granularity);
    }

  }

  public Filter initFilter(olapSelectionLayout selection){
   ...
   */
  private void splitTimeRange(granularityType granularity) throws ParseException {
  ...
  // process metaChunk
  public boolean process(MetaChunkRS metaChunk) {
