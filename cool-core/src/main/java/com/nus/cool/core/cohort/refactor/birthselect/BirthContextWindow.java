...
/**
 * BirthContextWindow is a sliding window based on adaptive queue. It keep a maxTimeWindow
 * and a time-based queue (LinkedList) When new (eventId, datetime) input,
 * this queue will automatically pop elements to keep the time
 * duration between the first and the last element is less than the
 * maxTimeWindow
 */
public class BirthContextWindow {
  ...
  public void put(Integer eventId, LocalDateTime date) {
    Preconditions.checkState(eventId < this.eventNum, "Input eventId is out of range");
  ...
  /**
   * Pop out the first element in queue and update the EventState.s
   */
  private void pop() {
    if (window.isEmpty()) {
      return;
