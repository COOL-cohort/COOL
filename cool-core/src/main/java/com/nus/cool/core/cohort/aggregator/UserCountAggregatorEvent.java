...
public class UserCountAggregatorEvent implements EventAggregator {
  ...
  public void ageAggregate(BitSet ageOffset, List<Integer> ageDelimiter,
    ...
    while (offset >= 0) {
      // determine the age
      while (offset >= ageDelimiter.get(age)) {
        age++;
      }
  ...
  public void ageAggregateMetirc(BitSet ageOffset, InputVector timeVec, int birthDay, int ageOff,
    ...
    while (toffset < ageEnd) {
      ...
      // if the cohort cell is for the metric, it consists of "measure", "max", "min",
      // "sum", "num"
      ...
      while (offset > 0 && offset < nextToffset) {
        hasActivity = true;
        fieldValue.skipTo(offset);
        int val = fieldValue.next();
        updateStats(val, cohortCell);
        offset = ageOffset.nextSetBit(offset + 1);
      }
      // add the measure by 1
      if (hasActivity) {
        ageMetrics.get(age).set(0, ageMetrics.get(age).get(0) + 1);
      }
