...
public class ExtendedCohortSelection implements Operator {
  ...
  private boolean ageActiveChunk;
  ...
  public void init(TableSchema tableSchema, ExtendedCohortQuery q) {
    ...
    if (ageRange == null || ageRange.isEmpty()) {
      ...
      ageRange.add(String.valueOf(Integer.MIN_VALUE) + "|" + String.valueOf(Integer.MAX_VALUE));
    }
    this.ageFilter = new AgeFieldFilter(ageRange);
    
    Map<String, FieldFilter> filterMap = this.ageByFilters;
    if (selectors != null) {
      for (ExtendedFieldSet fs : selectors) {
        String field = fs.getCubeField();
        filterMap.put(field, FieldFilterFactory.create(tableSchema.getField(field), fs,
            fs.getFieldValue().getValues()));
      }
    }
  ...
  public void process(MetaChunkRS metaChunk) {
    ...
    for (Map<String, FieldFilter> birthFilter : birthFilters) {
      for (Map.Entry<String, FieldFilter> entry : birthFilter.entrySet()) {
        ...
        if (minTriggerTime[idx] > 0) {
          userActiveCublet &= entry.getValue().accept(checkNotNull(metaField));
        } else {
    ...
    for (Map<String, FieldFilter> birthFilter : this.birthAggFilters) {
      for (Map.Entry<String, FieldFilter> entry : birthFilter.entrySet()) {
        userActiveCublet &= entry.getValue().accept(metaChunk.getMetaField(entry.getKey()));
      }
    }
    ...
    this.maxDate = TimeUtils.getDate(metaChunk
        .getMetaField(tableSchema.getActionTimeField(), FieldType.ActionTime).getMaxValue());
  }

  @Override
  public void process(ChunkRS chunk) {

    userActiveChunk = true;
    ageActiveChunk = true;
    ...
    for (int i = 0; i < birthFilters.size(); i++) {
      ...
      for (Map.Entry<String, FieldFilter> entry : filterMap.entrySet()) {
        ...
        if (minTriggerTime[i] > 0) {
          userActiveChunk &= entry.getValue().accept(field);
        } else {
          // todo: why do we need to run accept if this field is not considered
    ...
    for (Map<String, FieldFilter> birthFilter : this.birthAggFilters) {
      for (Map.Entry<String, FieldFilter> entry : birthFilter.entrySet()) {
        userActiveChunk &= entry.getValue().accept(chunk.getField(entry.getKey()));
      }
    }
    ...
    for (Map.Entry<String, FieldFilter> entry : ageFilters.entrySet()) {
      FieldRS field = chunk.getField(entry.getKey());
      ageActiveChunk &= entry.getValue().accept(field);
  ...
  /**
   * Find the next tuple where the given @param event is qualified with respect to the birth filter.
   *
   * @return the offset of the qualified tuple, or @param endOffset if not found
   */
  private int skipToNextQualifiedBirthTuple(int event, int fromOffset, int endOffset) {
    ...
    // If a common offset is found, i.e., passed == numFilter, then nextOffset (< endOffset) will be
    // returned.
    // If no common offset can be found, i.e., fromOffset >= endOffset, then nextOffset (>=
    // endOffset) will be returned.
    while (true) {
  ...
  /**
   * Check if the number of occurrence of each birth event follows the max/min trigger
   *  condition or not.
   *
  ...
  private boolean checkOccurrence(int e) {
    ...
    if (minTriggerTime[e] > occurrence
        || (maxTriggerTime[e] >= 0 && maxTriggerTime[e] < occurrence)) {
      return false;
    }
    return true;
  }
  ...
  /**
   * Check for each birth event if its aggregated value passes the aggregation filter or not.
   *
      ...
      for (Map.Entry<String, FieldFilter> entry : filters.entrySet()) {
        ...
        if (entry.getValue() instanceof SetFieldFilter) {
          ...
          if (value.getType() != ExtendedFieldSet.FieldValueType.AbsoluteValue) {
            filter.updateValues(getBirthAttribute(value.getBaseEvent(),
                tableSchema.getFieldID(value.getBaseField())));
          }
        ...
        for (BirthSequence.CohortField cf : be.getCohortFields()) {
          ...
          if (schema.getDataType() == DataType.String) {
            ...
            if (cf.isLogScale()) {
              ...
              int vint = v.intValue();
              level = Math.max(((v > vint) ? vint + 1 : vint), cf.getMinLevel());
  ...
  public String selectAgeActivities(int ageOff, int ageEnd, BitSet bs, BitSet ageDelimiters) {
    ...
    for (Map.Entry<String, FieldFilter> entry : ageFilters.entrySet()) {
      ...
      if (tableSchema.getField(entry.getKey()).getFieldType() == FieldType.Metric) {
        metricAgeFilterName = entry.getKey();
      }
  ...
  public boolean isUserActiveCublet() {
    return this.userActiveCublet;
  }

  public boolean isUserActiveChunk() {
    return this.userActiveChunk;
  }

  public boolean isAgeActiveChunk() {
    return this.ageActiveChunk;
  }
