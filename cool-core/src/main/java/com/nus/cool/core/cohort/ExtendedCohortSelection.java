...
public class ExtendedCohortSelection implements Operator {
    ...
    public void process(ChunkRS chunk, MetaChunkRS metaChunk) {
        ...
        for (int i = 0; i < birthFilters.size(); i++) {
            ...
            for (Map.Entry<String, FieldFilter> entry : filterMap.entrySet()) {
                if (tableSchema.getInvariantName2Id().keySet().contains(entry.getKey())) {
                    UserMetaFieldRS userMetaField = (UserMetaFieldRS) metaChunk.getMetaField(tableSchema.getUserKeyField(), FieldType.UserKey);
                    InputVector fieldValue = userMetaField.getUserToInvariant().get(tableSchema.getInvariantName2Id().get(entry.getKey()));
                    bUserActiveChunk &= entry.getValue().accept(fieldValue);
                    continue;
                }
    ...
    private int skipToNextQualifiedBirthTuple(int event, int fromOffset, int endOffset, List<Integer> userFieldVector) {
        ...
        while (true) {
            for (Map.Entry<String, FieldFilter> entry : birthFilters.get(event).entrySet()) {
                if (tableSchema.getInvariantType().contains(entry.getValue().getFieldType())) {
                    int fieldInvariantID = tableSchema.getInvariantName2Id().get(entry.getKey());
                    if (entry.getValue().accept(userFieldVector.get(fieldInvariantID))) {
                        nextOffset = fromOffset + 1;
                    }
                } else {
                    nextOffset = entry.getValue().nextAcceptTuple(fromOffset, endOffset);
                }
     ...
     * range
     */
    private void filterEvent(int eventId, int fromOffset, int endOffset, List<Integer> userFieldVector) {
        LinkedList<Integer> offset = eventOffset.get(eventId);
        while (fromOffset < endOffset) {
            fromOffset = skipToNextQualifiedBirthTuple(eventId, fromOffset, endOffset, userFieldVector);
    ...
    private int getUserBirthTime(int start, int end, List<Integer> userFieldVector) {
        BirthSequence seq = q.getBirthSequence();
        ...
        for (Integer e : sortedEvents) {
            ...
            if (window == null) {
                ...
                for (int i = 0; i < minTriggerTime[e]; i++) {
                    offset = skipToNextQualifiedBirthTuple(e, offset, end, userFieldVector);
                    if (offset == end)
                        return -1;
                ...
                while (startDay <= endDay) {
                    ...
                    filterEvent(e, offset, pos, userFieldVector);
                    offset = pos;
    ...
    public ExtendedCohort selectUser(int start, int end, List<Integer> userFieldVector, UserMetaFieldRS userMetaField) {
        ...
        int boff = this.getUserBirthTime(start, end, userFieldVector);
    ...
    private void filterAgeActivity(int ageOff, int ageEnd, BitSet bs, int userIndex, InputVector fieldIn, FieldFilter ageFilter) {
...
//		}
        ...
        fieldIn.skipTo(userIndex-1);
        int invariantValue=fieldIn.next();

        if (bs.cardinality() >= ((ageEnd - ageOff) >> 1)) {
            for (int i = ageOff; i < ageEnd; i++) {
                if (!ageFilter.accept(invariantValue)) {
                    bs.clear(i);
                }
            }
        } else {
...
//		}
    ...
    public String selectAgeActivities(int ageOff, int ageEnd, BitSet bs, BitSet ageDelimiters, int totalUsers, UserMetaFieldRS userMetaField) {
        checkArgument(ageOff < ageEnd);
        // enable the dimension-based ageby operator to be processed in the same way
        // as event-based ageby operator
        int fieldID = tableSchema.getFieldID(q.getAgeField().getField());
        ...
        for (Map.Entry<String, FieldFilter> entry : ageFilters.entrySet()) {
            ...
            if (tableSchema.getField(entry.getKey()).getFieldType() == FieldType.Metric)
                metricAgeFilterName = entry.getKey();
            if(tableSchema.getInvariantName2Id().keySet().contains(entry.getKey())){
                filterAgeActivity(ageOff, ageEnd, bs, totalUsers, userMetaField.getUserToInvariant().get(tableSchema.getInvariantName2Id().get(entry.getKey())) , ageFilter);
            }
            else{
                filterAgeActivity(ageOff, ageEnd, bs, chunk.getField(entry.getKey()).getValueVector(), ageFilter);
            }
