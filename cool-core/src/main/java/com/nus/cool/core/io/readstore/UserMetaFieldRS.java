...
/**
 * Read store of meta of a user key field in meta chunks.
 */
public class UserMetaFieldRS extends HashMetaFieldRS {
  ...
  /**
   * Return cublet global id mapping of an invariant field attacked to this user key field.
   */
  public int[] getInvariantGidMap(String invariantFieldName) {
    int[] ret = new int[this.count()];
    int invariantfieldIndex = this.invariantName2Id.get(invariantFieldName) - 1;
    InputVector invariantVector = this.userToInvariant.get(invariantfieldIndex);
