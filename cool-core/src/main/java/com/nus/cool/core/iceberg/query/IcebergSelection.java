...
/**
 * Iceberg selection class.
 */
public class IcebergSelection {

  private boolean activateCublet;
  ...
  // private IcebergQuery.GranularityType groupFieldsGranularity;
  ...
  // store multiple time range lower bounds and upper bounds, convert data into
  // integer
   ...
   * @return filter instance, parsed after checking selection type. (only supports
   *         and, or, filter)
   */
  ...
  public void init(TableSchema tableSchema, IcebergQuery query) throws ParseException {
    ...
    // this.groupFieldsGranularity = query.getGroupFieldsGranularity();
   ...
   * @return true: this cublet meet the requirements of seleciton, false: skip
   *         processing this.
   */
  ...
  public void process(MetaChunkRS metaChunk) {
    ...
    this.activateCublet = isAccepted && isProcess;
  }
  ...
  public ArrayList<TimeBitSet> process(ChunkRS dataChunk) {
    ...
    if (this.timeRange == null) {
      ...
      while (minKey > this.maxTimeRanges.get(timeRangeKey)) {
        timeRangeKey += 1;
      }

      // if all data in this dataChunk satisfied the current range at
      // key=timeRangeKeye
      if (minKey >= this.minTimeRanges.get(timeRangeKey)
          & maxKey <= this.maxTimeRanges.get(timeRangeKey)) {
        BitSet bv = new BitSet(dataChunk.records());
        bv.set(0, bv.size());
        ...
        for (int i = 0; i < timeInput.size(); i++) {
          ...
          if (time < this.minQueryTime) {
            continue; // this record is not in the range
          }
          if (time > this.maxQueryTime) {
            break; // the rest all greater than this.max. not in range, skip
          }
          if (time >= min && time <= max) {
            bitSets[timeRangeKey].set(i);
          } else {
            while (!(time >= this.minTimeRanges.get(timeRangeKey)
              && (time <= this.maxTimeRanges.get(timeRangeKey)))) {
              timeRangeKey += 1;
            }
  ...
  private BitSet select(SelectionFilter selectionFilter, ChunkRS chunk, BitSet bv) {
    ...
    if (selectionFilter.getType().equals(SelectionQuery.SelectionType.filter)) {
      ...
      // int gId =
      // this.metaChunk.getMetaField(selectionFilter.getDimension()).find(value);
  ...
  public boolean isactivateCublet() {
    return this.activateCublet;
  }
