...
public class MetaUserFieldWS extends MetaHashFieldWS {
  ...
  public MetaUserFieldWS(FieldType type, Charset charset, OutputCompressor compressor,
      int invatiantSize) {
  ...
  public void put(String[] tupleValue, List<FieldType> invariantType) {
    ...
    if (tupleValue.length == 1) {
      ...
      for (int i = 0; i < invariantSize; i++) {
        if (invariantType.get(i) == FieldType.ActionTime
            || invariantType.get(i) == FieldType.Metric) {
