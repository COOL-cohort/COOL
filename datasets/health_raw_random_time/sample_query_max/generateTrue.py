...
ret_path = os.path.join(dir,"query_result.json")
...
def generateAge(birth_date: datetime.date, action_date: datetime.date) -> int:
    ...
    if dtime.total_seconds() > 691199 or dtime.total_seconds() < 0:
        return -1
    return int(dtime.total_seconds()/86400)
...
if __name__ == '__main__':
    ...
    for user_key, birth_time in tqdm(userId2time.items()):
        ...
        for i in range(len(user_sub_data)):
            ...
            age = generateAge(birth_time, item["time"])
            if age == -1:
                continue
    ...
    for cohort, age_dict in result.items():
        for age, user_set in age_dict.items():
            cohortRet[cohort][age] = max(user_set)
